...
export const isSystemConfig = isType<SystemConfig>({
  ...
  arrayConfig: isNonEmptyArrayWithEachItem(
    isType({
  ...
  unionConfig: isOneOfTypes<
    boolean | NonEmptyArray<PositiveNumber> | NonEmptyString
  >(isBoolean, isNonEmptyArrayWithEachItem(isPositiveNumber), isNonEmptyString),
  complexUnion: isOneOfTypes<ComplexUser | DeepNestedData>(
    isComplexUser,
    isDeepNestedData,
  ),
  anyConfig: isAny,
  ...
  nullUnion: isNullOr(isNonEmptyString),
  undefinedUnion: isOneOfTypes<PositiveNumber | undefined>(
    isPositiveNumber,
    isEqualTo(undefined),
  ),
});
